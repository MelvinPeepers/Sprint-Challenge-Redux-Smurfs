...
- [x] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux.
      What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

---

Actions are information that we send (dispatch) to our Redux store. They are the only source of information for your store.

Reducers are a pure function that take the previous state and an action and then sends the updated data to state.

## The store contains the state of the application and cannot be changed. The state (data) of your app is stored in an single state object.

- [x] What is the difference between Application state and Component state? When would be a good time
      to use one over the other?

---

With application state we never write to our store object. We simply clone the state object, modify the clone, and replace the original state with the new copy. We never mutate the original object. Additionally, any component anywhere in your app can have access to it.

Component state is only available within a specific component and can only be updated within that component than passed does to children via props.

## Application state is best to use when you have a lot of data.

- [x] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

---

## Redux-Thunk is a function that returns another function. Thunk allows us to return functions from our action creators and not just actions. It allows us to return a function instead of an object.
